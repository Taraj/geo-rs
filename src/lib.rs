...
#[napi]
pub fn is_point_on_line_fast(encoded_polyline: String, p_lat: f64, p_lon: f64, margin: f64) -> bool {
    ...
    for window in points.windows(2) {
        ...
        if p_lat < min_lat || p_lat > max_lat { continue; }

        let min_lon = p1.1.min(p2.1) - margin_lon;
        let max_lon = p1.1.max(p2.1) + margin_lon;
        if p_lon < min_lon || p_lon > max_lon { continue; }
